...
function_hash['1-7'] = function(data, header, command_set, command, socket) { 
  variably_sized_data_types = {
    fieldIDSize:         parse_bytes_to_int(data, 11, 14),
    methodIDSize:        parse_bytes_to_int(data, 15, 18),
    objectIDSize:        parse_bytes_to_int(data, 19, 22),
    referenceTypeIDSize: parse_bytes_to_int(data, 23, 26),
    frameIDSize:         parse_bytes_to_int(data, 27, 30)
  };
};
...
var parse_header = function(data) {  
  ...
  var error_code  = parse_bytes_to_int(data, 9, 10);
  var command_set = parse_bytes_to_int(data, 9, 9);
  var command     = parse_bytes_to_int(data, 10, 10);
