...
function_hash['1-2'] = function(raw_data, header, command_set, command, socket) { 
  
  // command = jdwp.classes_by_signature_command('"LHelloWorld;"');
...
function_hash['1-7'] = function(raw_data, header, command_set, command, socket) { 
  ...
  // var command = jdwp.resume_events_command();
  // parse_header(command);
  // socket.write(command.prepareForSocketTransmission()); // first suspend by the debugger

  // setTimeout(function(){
  //   var command = jdwp.get_all_classes_command();
  //   parse_header(command);
  //   socket.write(command.prepareForSocketTransmission());
  // }, 1000);

  var command = jdwp.classes_by_signature_command('LHelloWorld;');
  parse_header(command);
  socket.write(command.prepareForSocketTransmission());
...
var parse_header = function(raw_data) {  
  ...
  console.log('Data:        ' + data.binaryToString());
  console.log();
  
  if (command_set == 0 && error_code != 0) {
    switch(error_code) {
      case 113:
        console.log('113 - An unexpected internal error has occurred.');
        break;
    }
  }
